...
function Header() {
	...
	const handleRegister = () => {
		fetch('https://morningnews-backend-pi.vercel.app/users/signup', {
			method: 'POST',
	...
	const handleConnection = () => {
		...
		fetch('https://morningnews-backend-pi.vercel.app/users/signin', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
	...
	if (user.token) {
		userSection = (
			<div className={styles.logoutSection}>
				...
				<FontAwesomeIcon onClick={() => dispatch(removeAllHiddenArticles())} className={styles.userSection} icon={faEye} />
			</div>
		);
	} else {
		if (isModalVisible) {
